 ...
 *
 * For JS-heavy sites use the shared browser engine instead:
 *   `import { loadPage } from "@neuralsearchindex/scraper-provider-sdk/browser"`
 * — the SAME camoufox+chromium/CDP + Cloudflare-clearing engine the central scraper uses (requires
 * the optional `playwright-core` peer + a reachable `PLAYWRIGHT_VNC_CDP_URL`). Kept on a subpath so
 * deterministic providers that only `fetchHtml` pull in neither playwright nor a browser.
 */
